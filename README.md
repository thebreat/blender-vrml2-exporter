...
Use the included distributable archive named `vrml2_exporter-0.6.2.zip`. **Do not extract it first.**
...
5. Select `vrml2_exporter-0.6.2.zip`.
...
| **Two-Sided Faces** | Writes `solid FALSE` on every exported geometry so VRML viewers render both sides of its faces. Disabled by default; it is most useful for thin or open meshes and does not add thickness or repair mesh topology. |
